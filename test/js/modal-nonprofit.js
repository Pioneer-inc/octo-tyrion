...
/* This is the NONPROFIT version of glyder.js. */
...
// MOVE THESE TWO FUNCTIONS TO cpModal for vendor specific code
...
function glyder_check_subscription(user_id, vendor_id) {
	...
	if (valid_until && valid_until >= n) {
		if (localStorage['LastPurchase'] == item) {
			// Temporary fix for KUOW demo. Prior message: 'Thank you for your donation!'
			// This message currently displays when user clicks "Day Pass" button.
			$('#div_access_message').html('We value your support! (msg01)');				
		} else {
			// Temporary fix for KUOW demo. Prior message: 'Your account includes access to this premium content.<br>Thank you for your prior donation!' 
			$('#div_access_message').html('We value your support! (msg02)');
		}
		return true;
	}
	return false;
...
function glyder_check_item_purchased(user_id, vendor_id, item_id) {
	...
	if (valid_until && valid_until != "0") {
		if (localStorage['LastPurchase'] == item) {
			// Temporary fix for KUOW demo. Prior message: 'Thank you for your donation!' 
			// This msg is currently displaying when user clicks to donate for a specific item AND also displays when user clicks the 'No, Thanks' button. Kinda wrong. Need new function and message to display when user clicks 'No, Thanks' button for nonprofits.
			$('#div_access_message').html('We value your support! (msg03)');				
		} else {
			// Temporary fix for KUOW demo. Prior message: 'You previously donated to support this item. Thank&nbsp;you!' 
			$('#div_access_message').html('We value your support! (msg04)');
		}
		return true;				
	}
	return false;
...
function glyderBuyItem() {
	...
	$("#accessModal").modal("hide");
	// Temporary fix for KUOW demo. Prior message: 'Thank you for your donation!'	
	$('#div_access_message').html('We value your support! (msg05)');				
	return true;
}

function glyderBuyItemAndGo() {
	if (glyderBuyItem()) glyder_show_or_redirect();
	else alert("Item donation payment failed.");
}
...
function glyderBuyDaypass() {
	...
	$("#accessModal").modal("hide");
	// Temporary fix for KUOW demo. Prior message: 'Thank you for your donation!'	
	$('#div_access_message').html('We value your support! (msg06)');				
	return true;
}

function glyderBuyDaypassAndGo() {
	if (glyderBuyDaypass()) glyder_show_or_redirect();
	else alert("Day Pass donation payment failed.");
}
		...
		return;
	  	...
	  	if (data.getNumberOfRows() < 1) {
			...
			if (glyder_item_cost < 0.000001) {
				// Temporary fix for KUOW demo. Prior message: 'Your account includes access to this premium content.<br>Thank you for your prior donation!'
				$('#div_access_message').html('We value your support! (msg07)');				
			}
